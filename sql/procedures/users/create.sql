...
BEGIN
    ...
    IF (NOT ISNULL(@firstname) AND NOT ISNULL(@lastname)) THEN
		EXECUTE prepared_stmt USING @username, @firstname, @lastname, @email;
	ELSEIF (NOT ISNULL(@firstname) AND ISNULL(@lastname)) THEN
		EXECUTE prepared_stmt USING @username, @firstname, @email;
	ELSEIF (ISNULL(@firstname) AND NOT ISNULL(@lastname)) THEN
		EXECUTE prepared_stmt USING @username, @lastname, @email;
    ELSE EXECUTE prepared_stmt USING @username, @email; END IF;
